...
from heroesEmojis import emoji
import discord
	...
	if len(draftList)>1:
		...
		if len(draftList) in [2,3,4,5,11,12]:#Numbers are the bans
			if hero in banEmojis.keys():
				...
				lastDraftMessageDict[channel]=await channel.send(output+'```',file=discord.File('resources/Emojis/'+hero+' sad'+fileExtension))
				return
		else:
			if channel in lastDraftMessageDict:
				try:await lastDraftMessageDict[channel].delete()
				except:pass
			lastDraftMessageDict[channel]=await channel.send(output+'```',file=discord.File('resources/Emojis/'+hero+' happy'+fileExtension))

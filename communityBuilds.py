...
from functionsPrint import getHeroes
from heroesAliases import aliases
...
async def guide(hero,channel):
	output=''
	with open('resources/Text files/otherBuilds.txt','r') as f:
		for i in f:
			if hero.lower() in i.lower():
				authorAndLink=i.split('; ')[1]
				[author,link]=authorAndLink.split(': ')
				output+=author+': <'+link.replace('\n','')+'>\n'
	with open('resources/Text files/mindHawkBuilds.txt','r') as f:
		for i in f:
			if await trimForIcyVeinsAndPsionicStorm(hero) in i:
				output+='MindHawk: <'+i.replace('\n','')+'>\n'
	
	with open('resources/Text files/icyVeinsHeroes.txt','r') as f:
		for i in f:
			if hero==aliases(i):
				await channel.send(output+'Icy Veins: <https://www.icy-veins.com/heroes/'+(await trimForIcyVeinsAndPsionicStorm(hero)).replace('kelthuzad','kel-thuzad').replace('etc','e-t-c')+'-build-guide>')#<> prevents thumbnails.
				return

	with open('resources/Text files/elitesparkleBuilds.txt','r') as f:
		for i in f:
			if await trimForIcyVeinsAndPsionicStorm(hero) in i:
...
def updateBuilds():
	...
	with open('resources/Text files/elitesparkleBuilds.txt','w+') as f:

...
from blizztrack import BlizztrackMonitor
...
#		await client.get_channel(DiscordChannelIDs['LoggingChannel']).send('`{}`'.format(loggingMessage))
	...
	for text in texts:
		...
		if command in versionAliases:
			...
			blizztrack_versions = await client.blizztrackMonitor.get_versions()
			...
			health_checks=client.blizztrackMonitor.validate_versions(blizztrack_versions)
			health_summary='\n'.join([
				f"{name}: {'ok' if passed else 'fail'}" for name,passed in health_checks
			])
			await message.channel.send(
				'Current live version: '+live_version+'\n'
				+'Current test version: '+test_version+'\n'
				+'Probius is version: '+version+'\n'
				+'BlizzTrack health checks:\n'+health_summary
			)
			continue
...
#		self.proxyEmojis={}
		...
		self.blizztrackMonitor=BlizztrackMonitor()
...
#		self.welcomeMessage='Please read '+self.rulesChannel.mention+' and type here your **`Region`, `Rank`, and `Preferred Colour`**, separated by commas, to get sorted and unlock the rest of the channels <:OrphAYAYA:657172520092565514>'
	...
	async def check_blizztrack_versions(self,announce_if_first_run=False):
		current_versions=await self.blizztrackMonitor.get_versions()
		if not current_versions:
			logging.warning('BlizzTrack check failed: no data returned')
			return
		for check_name,passed in self.blizztrackMonitor.validate_versions(current_versions):
			if not passed:
				logging.warning('BlizzTrack health check failed: %s', check_name)
			else:
				logging.info('BlizzTrack health check passed: %s', check_name)

		previous_state=self.blizztrackMonitor.state if isinstance(self.blizztrackMonitor.state,dict) else {}
		probius_channel=self.get_channel(DiscordChannelIDs['Probius'])
		if not previous_state:
			self.blizztrackMonitor.state=current_versions
			self.blizztrackMonitor.write_state(current_versions)
			if announce_if_first_run and probius_channel is not None:
				await probius_channel.send('blizztrack initial versions: '+' | '.join(self.blizztrackMonitor.summary_lines(current_versions)))
			return

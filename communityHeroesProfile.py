...
from heroesTalents import readVersion

gamemode='qm'
timeFrameType='major'

def currentPatch():
	v = readVersion('.hversion')
	parts = v.split('.')
...
async def printHeroes(heroes,gamemode,totalGames,channel):
	output=['Score results QM patch '+patch+', '+str(totalGames)+' games total from <https://heroesprofile.com/>']
	if gamemode=='qm':
